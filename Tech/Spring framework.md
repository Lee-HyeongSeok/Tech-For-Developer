...
- **DOM(Document Object Model)** 
  -  웹 페이지를 객체로 표현하는 모델을 의미
  - 브라우저마다 DOM을 구현하는 방식이 다름
  - **DOM을 이용한 화면 렌더링**
    1. **브라우저**는 HTML 태그를 파싱하여 **DOM Tree**를 구성, 스타일시트에서 CSS를 파싱하여 **스타일 규칙**들을 만들어냄
    2. **DOM Tree**와 **스타일 규칙** 두 가지가 합쳐져서 **렌더 트리**를 형성한다
       - **렌더 트리**는 화면에 반영되는 것만 가지고 있음

- **가상 DOM(Vitual Document Object Model)**
  - **기존 DOM**에서는 화면 변경 사항들을 DOM을 직접 조작했음
    - DOM Tree가 수정될 때마다 렌더 Tree가 실시간으로 갱신되는 단점 존재
  - **가상 DOM**은 이런 불필요한 렌더링 횟수를 최소화 시킬 수 있음
  - 화면에 변화가 생길 때마다 실시간으로 DOM Tree를 수정하지 않고 변경사항이 모두 반영된 가상 DOM을 만들어 낸 후 한 번만 DOM을 수정한다.
  - **가상 DOM을 활용한 프론트 엔드 프레임워크** : 리액트, 뷰, 앵귤러